...
struct IPFSWebserviceAPI: IPFSAPI {
    ...
    func add(file: URL, completion: @escaping ((_ response: AddResponse?, _ error: Error?) -> Void)) {
        os_log("Adding file to IPFS: %{public}s", log: Loggers.network, type: .info, file.absoluteString)
    ...
    func cat(arg: String, completion: @escaping ((_ data: Data?, _ error: Error?) -> Void)) {
        os_log("Catting object from IPFS: %{public}s", log: Loggers.network, type: .info, arg)

        DefaultAPI.cat(arg: arg) { (data, error) in
            if data != nil {
                os_log("Cat of %{public}s from IPFS returned %d bytes", log: Loggers.network, type: .info,
                       data?.count ?? 0)
            } else if let error = error {
                os_log("Failed to cat %{public}s from IPFS. %{public}s", log: Loggers.network, type: .error,
                       self.describeError(error))
            }
    ...
    func publish(arg: String, key: String?,
        ...
        os_log("Publishing file %{public}s under key %{public}s", log: Loggers.network, type: .info,
    ...
    func resolve(arg: String, recursive: Bool?,
        ...
        os_log("Resolving hash %{public}s", log: Loggers.network, type: .info, arg)
    ...
    func removeKey(arg: String, completion: @escaping ((_ response: RemoveKeyResponse?, _ error: Error?) -> Void)) {
        os_log("Removing key %{public}s", log: Loggers.network, type: .info, arg)

        DefaultAPI.removeKey(arg: arg) { (response, error) in
            if response != nil {
                os_log("Removed key %{public}s", log: Loggers.network, type: .info, arg)
            } else {
                os_log("Failed to removed key %{public}s. %{public}s", log: Loggers.network, type: .info,
                       arg, self.describeError(error))
            }
