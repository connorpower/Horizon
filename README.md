...
Xcode projects are typically autogenerated based on the Swift Package Manager's
manifest file. Currently this works for frameworks, static libraries and
command line apps – Cocoa apps are not yet supported. As such, the command
`swift package generage-xcodeproj` should not be run. It was run once to
generate the initial project structure, but the Horizon Cocoa App has been
manually added and configured.
